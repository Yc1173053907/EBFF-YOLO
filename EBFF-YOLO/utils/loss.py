...
from utils.general import bbox_mpdiou #zz add
from utils.general import bbox_soft_mpdiou #zz add
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # iou = bbox_mpdiou(pbox.T, tbox[i], x1y1x2y2=False, mpdiou_hw=pi.size(2) ** 2 + pi.size(3) ** 2, grid=torch.stack([gj, gi]))  # iou(prediction, target) .T 矩阵转置 #zz add
                # iou = bbox_soft_mpdiou(pbox.T, tbox[i], x1y1x2y2=False, mpdiou_hw=pi.size(2) ** 2 + pi.size(3) ** 2, grid=torch.stack([gj, gi]))  # iou(prediction, target) .T 矩阵转置 #zz add添加mpdiou
                

                nwd = wasserstein_loss(pbox, tbox[i]).squeeze()#zz add
                iou_ratio = 1  
                # iou_ratio = 0.5   #zz add 添加whiou分支
...
def wasserstein_loss(pred, target, eps=1e-7, constant=12.8):
    ...
    wasserstein_2 = center_distance**2 + wh_distance*4
