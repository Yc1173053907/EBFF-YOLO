...
import os
import pandas as pd
PATH = 'E:/project_zhangzhen/SuperYOLO-improve/datasets/VEDAI/' #chanhe the path firstly (PATH TO dataset)dddd
...
def makelabels():
    label_path = PATH + 'VEDAI/labels/'
    save_path = PATH + 'VEDAI/labels_new/'
...
def changepath():
    for i in ['01','02','03','04','05','06','07','08','09','10']:
        path = PATH + 'class_datasets/fold{}.txt'.format(i)
        img_path = PATH + 'VEDAI_1024/images/'
        write_path=(PATH + 'class_datasets/fold{}_write.txt').format(i)
        ...
        path = PATH + 'class_datasets/fold{}test.txt'.format(i)
        img_path = PATH + 'VEDAI_512/images/'
        write_path=PATH + 'class_datasets/fold{}test_write.txt'.format(i)
        with open(path, "r") as file:
            img_files = file.readlines()
            for j in range(len(img_files)):
                img_files[j] =  img_path + img_files[j].rstrip()
